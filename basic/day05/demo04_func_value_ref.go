...
func main() {
	...
	*/
	...
	arr1 := [4] int{1,2,3,4}
	fmt.Println("调用函数前，arr 的值为：",arr1)  //[1 2 3 4]
	printArr(arr1)
	fmt.Println("调用函数后，arr 的值为：",arr1)  //[1 2 3 4]


	s1 := [] int{1,2,3,4}
	fmt.Println("调用函数前，s1 的值为：",s1)  //[1 2 3 4]
	printSlice(s1)
	fmt.Println("调用函数后，s1 的值为：",s1)  //[100 2 3 4]
}

func printArr(arr2 [4]int) {
...
func printSlice(s2 []int) {
	...
	s2[0] = 100
	fmt.Println("函数中，切片的数据修改后：", s2) //[100 2 3 4]}
}
